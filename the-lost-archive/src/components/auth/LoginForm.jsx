            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-5 max-w-md mx-auto">
                    ...
                    <div>
                        ...
                        <input name="password" type="password" required placeholder="••••••••" className="w-full bg-transparent border-b border-stone-300 py-2 focus:outline-none focus:border-amber-700 font-serif transition-colors" />
                    </div>
                </div>

                <button disabled={loading} className="w-full bg-stone-900 text-[#F5F2ED] py-4 text-[10px] uppercase tracking-[0.3em] font-bold hover:bg-amber-950 transition-all flex justify-center items-center gap-2">
                    {loading ? <><Loader2 className="animate-spin" size={16} /> Verifying...</> : "Authorize Access"}
                </button>

                <footer className="mt-8 pt-6 border-t border-stone-100 text-center">
                    <p className="text-xs text-stone-500 font-serif">
                        New explorer? <Link href={`/register?callbackUrl=${callBack}`} className="text-amber-800 hover:text-stone-900 underline underline-offset-4 transition-colors font-bold">Register Now</Link>
                    </p>
                </footer>
                <SocialButton />
            </form>
